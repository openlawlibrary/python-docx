...
class ST_Border(XsdString):
    ...
    def validate(cls, value):
        cls.validate_string(value)
        if value not in ST_Border.valid_values:
            raise ValueError(
                "must be one of %s, got '%s'" % (ST_Border.valid_values, value)
            )
...
class ST_BrClear(XsdString):
    ...
    def validate(cls, value):
        cls.validate_string(value)
        if value not in ST_BrClear.valid_values:
            raise ValueError(
                "must be one of %s, got '%s'" % (ST_BrClear.valid_values, value)
            )
