...
class ST_Border(XsdStringEnumeration):
    """Valid values for `w:top`/`w:left`/`w:bottom`/`w:right`/etc `@w:val` in a border
    context, e.g. `w:tblBorders`/`w:tcBorders`."""
...
class ST_TextDirection(XsdStringEnumeration):
    """Valid values for `w:textDirection/@w:val`."""

    _members = ("btLr", "lrTb", "lrTbV", "tbLrV", "tbRl", "tbRlV")
